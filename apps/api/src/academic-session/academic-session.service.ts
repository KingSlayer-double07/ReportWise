...
export class AcademicSessionService {
  ...
  async createTerm(schoolSlug: string, dto: CreateTermDto) {
    ...
    if (existingTerm) {
      throw new ConflictException(
        'This academic term already exists in this session.',
      );
    }

    //Ensure term dates are within session dates
    if (
      new Date(dto.startDate) < new Date(session.startDate) ||
      new Date(dto.endDate) > new Date(session.endDate)
    ) {
      throw new ConflictException(
    ...
    if (termCount >= 3) {
      throw new ConflictException(
        'An academic session can only have a maximum of 3 terms.',
      );
    }
