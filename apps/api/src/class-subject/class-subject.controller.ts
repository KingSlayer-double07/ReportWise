...
export class SubjectController {
  ...
  @ApiResponse({
    status: 200,
    description: 'The subject name has been successfully updated.',
  })
  @ApiResponse({ status: 400, description: 'Invalid input data.' })
  ...
  updateSubject(
    ...
    @Body() dto: { subjectName: string },
  ) {
    return this.svc.updateSubject(
      req.user.schoolSlug,
      subjectId,
      dto.subjectName,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Deletes a subject from the school. This action cannot be undone.',
  })
  @ApiBearerAuth()
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 409,
    description:
      'Subject cannot be deleted because it is assigned to one or more classes.',
  })
  @Delete(':subjectId')
  @Roles(Role.ADMIN)
...
export class ClassSubjectController {
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves a list of all classes in the school along with their assigned subjects.',
  })
  @ApiBearerAuth()
  @ApiResponse({
    status: 200,
  ...
  @ApiOperation({
    ...
    description:
      'Returns a list of all classes along with the subjects assigned to each class.',
  })
  @ApiBearerAuth()
  @ApiResponse({
    status: 200,
    description: 'Returns a list of classes with their subjects.',
  })
  @ApiResponse({ status: 404, description: 'No classes found.' })
  @Get()
  @Roles(Role.ADMIN, Role.TEACHER)
  ...
  @ApiOperation({
    ...
    description:
      'Returns the details of a specific class along with the subjects assigned to it.',
  })
  @ApiBearerAuth()
  @ApiResponse({
    status: 200,
    description: 'Returns the class details with its subjects.',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'The subject has been successfully assigned to the class.',
  })
  @ApiResponse({ status: 400, description: 'Invalid input data.' })
  @ApiResponse({ status: 404, description: 'Class or subject not found.' })
  @ApiResponse({
    status: 409,
    description: 'Subject is already assigned to the class.',
  })
  ...
  @ApiResponse({
    status: 200,
    description: 'The subject has been successfully removed from the class.',
  })
  @ApiResponse({
    status: 404,
  ...
  @ApiResponse({
    status: 200,
    description: 'The subjects have been successfully assigned to the class.',
  })
  @ApiResponse({ status: 400, description: 'Invalid input data.' })
  @ApiResponse({ status: 404, description: 'Class or subject not found.' })
