...
export class ClassSubjectService {
    ...
    async validateSubjectName(subjectName: string) {
        ...
        if (subjectName.length < 3) {
            throw new BadRequestException('Subject name must be at least 3 characters long.');
        }
        if (subjectName.length > 50) {
            throw new BadRequestException('Subject name cannot exceed 50 characters.');
        }
        if (/[^a-zA-Z\s]/.test(subjectName)) {
            throw new BadRequestException('Subject name can only contain letters and spaces.');
        }
    }
    ...
    async createSubject(schoolSlug: string, subjectName: string) {
        ...
        if (existingSubject.length > 0) {
            throw new ConflictException(`Subject with name "${subjectName}" already exists. Subject names must be unique.`);
        }
    ...
    async updateSubject(schoolSlug: string, subjectId: string, newName: string) {
        ...
        if (duplicateSubject.length > 0) {
            throw new ConflictException(`Subject with name "${newName}" already exists. Subject names must be unique.`);
        }
    ...
    async deleteSubject(schoolSlug: string, subjectId: string) {
        ...
        if (assignedClasses.length > 0) {
            throw new ConflictException(`Subject with ID "${subjectId}" is assigned to one or more classes and cannot be deleted.`);
        }
        // Delete the subject
        await retry(() =>
            withTenant(this.prisma, schoolSlug, (tx) =>
    ...
    async assignSubjectFunction(schoolSlug: string, classId: string, dto: AssignSubjectDto) {
        await retry(() =>
            withTenant(this.prisma, schoolSlug, (tx) =>
            tx.$executeRaw`
    ...
    async validateSubjectAssignmentFunction(schoolSlug: string, classId: string, dto: AssignSubjectDto) {
        // Validate stream
        if (Stream[dto.stream] === undefined) {
            throw new BadRequestException(`Stream "${dto.stream}" is not valid. Valid streams are: ${Object.keys(Stream).join(', ')}.`);
        }
        ...
        if (classLevel.includes('JSS') && dto.stream !== 'NONE') {
        ...
        if (existingAssignment.length > 0) {
            throw new ConflictException(`Subject with ID "${dto.subjectId}" is already assigned to class with ID "${classId}" and stream "${dto.stream}".`);
        }
    }
    
    /** POST /classes Assign subjects to a class */
    async assignSubjectToClass(schoolSlug: string, classId: string, dto: AssignSubjectDto) {
        // Validate Input
    ...
    async removeSubjectFromClass(schoolSlug: string, classSubjectId: string) {
        ...
        if (existingAssignment.length === 0) {
            throw new NotFoundException(`Class-subject assignment with ID "${classSubjectId}" not found.`);
        }
    ...
    async bulkAssignSubjects(schoolSlug: string, classId: string, assignments: AssignSubjectDto[]) {
        // Check if class exists
        const existingClass: any[] = await retry(() =>
            withTenant(this.prisma, schoolSlug, (tx) =>
            tx.$queryRaw`
